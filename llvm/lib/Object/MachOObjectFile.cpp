...
ArrayRef<uint8_t> MachOObjectFile::getDyldInfoExportsTrie() const {
  ...
  if (DyldExportsTrieLoadCmd) {
    auto ExportsTrieOrErr =
      getStructOrErr<MachO::linkedit_data_command>(*this, DyldExportsTrieLoadCmd);
    if (!ExportsTrieOrErr)
      return None;
    MachO::linkedit_data_command ExportsTrie = ExportsTrieOrErr.get();
    const uint8_t *Ptr =
        reinterpret_cast<const uint8_t *>(getPtr(*this, ExportsTrie.dataoff));
    return makeArrayRef(Ptr, ExportsTrie.datasize);
  }
